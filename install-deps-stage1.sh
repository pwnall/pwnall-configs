...
if command -v apt >/dev/null 2>&1; then
  ...
  # Text processing used by AI agents and scripts.
  sudo apt install -y mawk grep ripgrep
...
# Locale files for terminals.
sudo locale-gen en_US.UTF-8
sudo update-locale LANG=en_US.UTF-8

# Directory structure for SSH-ing into other machines.
mkdir -p ~/.ssh/sockets
chmod 700 ~/.ssh
